...
function serialize(store: Store) {
  return {
    ...
    groupCount: store.filteredGroups.length,
    selectedCount: store.selectedCount,
    selectedSize: store.selectedSize,
    groups: store.filteredGroups.map((group) => ({
      size: group.size,
...
async function clean(store: Store, args: CleanArgs) {
  ...
  const failed = new Set(result.errors)
  const deleted = new Set(args.paths.filter((path) => !failed.has(path)))

  store.duplicateGroups = store.duplicateGroups
    .map((group) => ({
      ...group,

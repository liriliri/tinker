...
class Explorer {
  ...
  constructor(id: string, session: ISftpSessionConfig) {
    this.id = id
  ...
  pushHistory(nextPath: string) {
    if (this.history[this.historyIndex] === nextPath) return
